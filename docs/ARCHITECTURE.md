...
- `src/styles/index.css` compone los estilos con capas ordenadas; `src/styles/legacy.css` mantiene la base visual, `src/styles/workspace-layout.css` concentra la matriz de layout, `src/styles/panel-states.css` concentra estados de paneles, `src/styles/controls.css` concentra estados de controles y `src/styles/overlays.css` concentra dialogs y overlays antes de aplicar `src/styles/mobile.css` como capa responsive.
